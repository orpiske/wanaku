...
import {buildCliDownloadInfo, CliDownloadInfo, normalizeReleaseChannel} from "./downloads";
import {DownloadsTable} from "./DownloadsTable";
import {useInfo} from "../../hooks/api/use-info";

export const DownloadsPage: React.FC = () => {
  const {getInfo} = useInfo();
  // Render immediately with the fallback model, then refine once the info API responds.
  const [info, setInfo] = useState<CliDownloadInfo>(() => buildCliDownloadInfo());
  ...
  useEffect(() => {
    ...
    getInfo()
      .then((res) => {
        ...
        setInfo(buildCliDownloadInfo(res.data.version, normalizeReleaseChannel(res.data.channel)));
      })
      .catch(() => {
        // Keep the fallback model if the info API is unavailable.
      });
    return () => {
      active = false;
    };
      ...
      <div id="page-content">
        <Grid narrow>
          <Column lg={16} md={8} sm={4}>
            <DownloadsTable downloads={info.downloads} />
          </Column>
